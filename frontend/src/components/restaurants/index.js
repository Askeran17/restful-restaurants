...
const Restaurants = () => {
  ...
  const onStarRestaurant = async (id) => {
    ...
    // Get updated list of starred restaurants from server
    const starred = await import("../../api/starredRestaurants");
    const starredRestaurants = await starred.getStarredRestaurants();
    dispatch({ type: "LOADED_STARRED_RESTAURANTS", payload: starredRestaurants });
  };
